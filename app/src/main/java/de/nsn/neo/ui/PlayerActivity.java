...
public final class PlayerActivity extends Activity {
    ...
    private void addTvControls(){
        controls=new LinearLayout(this);controls.setOrientation(LinearLayout.VERTICAL);controls.setGravity(android.view.Gravity.CENTER);controls.setVisibility(View.INVISIBLE);controls.setPadding(NsnViews.dp(this,BuildConfig.IS_TV?20:8),NsnViews.dp(this,BuildConfig.IS_TV?10:6),NsnViews.dp(this,BuildConfig.IS_TV?20:8),NsnViews.dp(this,BuildConfig.IS_TV?10:6));controls.setBackgroundColor(Color.argb(BuildConfig.IS_TV?210:225,0,0,0));
        LinearLayout progressRow=new LinearLayout(this);progressRow.setGravity(android.view.Gravity.CENTER_VERTICAL);timeLabel=NsnViews.text(this,"00:00:00 / 00:00:00",BuildConfig.IS_TV?16:13,Color.WHITE);progressRow.addView(timeLabel);timeline=new SeekBar(this);timeline.setMax(1000);timeline.getProgressDrawable().setTint(getColor(de.nsn.neo.R.color.nsn_red));timeline.getThumb().setTint(getColor(de.nsn.neo.R.color.nsn_red));timeline.setFocusable(true);timeline.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar bar,int value,boolean user){if(user){long duration=resolved.get()?playback.durationMs():webDurationMs;seekAbsolute(duration*value/1000);}}public void onStartTrackingTouch(SeekBar b){}public void onStopTrackingTouch(SeekBar b){}});LinearLayout.LayoutParams timelineParams=BuildConfig.IS_TV?new LinearLayout.LayoutParams(NsnViews.dp(this,700),-2):new LinearLayout.LayoutParams(0,-2,1f);progressRow.addView(timeline,timelineParams);controls.addView(progressRow);
        controlButtons=new LinearLayout(this);controlButtons.setOrientation(BuildConfig.IS_TV?LinearLayout.HORIZONTAL:LinearLayout.HORIZONTAL);controlButtons.setGravity(android.view.Gravity.CENTER);controls.addView(controlButtons,new LinearLayout.LayoutParams(-1,-2));
    ...
    private void addControl(String label,Runnable action){TextView button=NsnViews.text(this,label,BuildConfig.IS_TV?18:14,Color.WHITE);button.setFocusable(true);button.setClickable(true);button.setGravity(android.view.Gravity.CENTER);button.setSingleLine(true);button.setPadding(NsnViews.dp(this,BuildConfig.IS_TV?20:10),NsnViews.dp(this,BuildConfig.IS_TV?14:10),NsnViews.dp(this,BuildConfig.IS_TV?20:10),NsnViews.dp(this,BuildConfig.IS_TV?14:10));button.setBackgroundColor(Color.rgb(32,32,32));button.setOnFocusChangeListener((v,f)->{v.setBackgroundColor(f?Color.rgb(235,0,35):Color.rgb(32,32,32));v.animate().scaleX(f?1.04f:1f).scaleY(f?1.04f:1f).setDuration(100).start();});button.setOnClickListener(v->action.run());LinearLayout.LayoutParams lp=BuildConfig.IS_TV?new LinearLayout.LayoutParams(-2,-2):new LinearLayout.LayoutParams(0,-2,1f);lp.setMargins(NsnViews.dp(this,2),0,NsnViews.dp(this,2),0);controlButtons.addView(button,lp);}
