...
public final class PlayerActivity extends Activity {
    ...
    private void addTvControls(){
        controls=new LinearLayout(this);controls.setOrientation(LinearLayout.VERTICAL);controls.setGravity(android.view.Gravity.CENTER);controls.setVisibility(View.INVISIBLE);controls.setPadding(NsnViews.dp(this,20),NsnViews.dp(this,10),NsnViews.dp(this,20),NsnViews.dp(this,10));controls.setBackgroundColor(Color.argb(210,0,0,0));
        LinearLayout progressRow=new LinearLayout(this);progressRow.setGravity(android.view.Gravity.CENTER_VERTICAL);timeLabel=NsnViews.text(this,"00:00:00 / 00:00:00",16,Color.WHITE);progressRow.addView(timeLabel);timeline=new SeekBar(this);timeline.setMax(1000);timeline.setFocusable(true);timeline.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar bar,int value,boolean user){if(user){long duration=resolved.get()?playback.durationMs():webDurationMs;seekAbsolute(duration*value/1000);}}public void onStartTrackingTouch(SeekBar b){}public void onStopTrackingTouch(SeekBar b){}});int timelineWidth=de.nsn.neo.BuildConfig.IS_TV?700:220;progressRow.addView(timeline,new LinearLayout.LayoutParams(NsnViews.dp(this,timelineWidth),-2));controls.addView(progressRow);
        controlButtons=new LinearLayout(this);controlButtons.setGravity(android.view.Gravity.CENTER);controls.addView(controlButtons);
    ...
    private void addControl(String label,Runnable action){TextView button=NsnViews.text(this,label,18,Color.WHITE);button.setFocusable(true);button.setClickable(true);button.setPadding(NsnViews.dp(this,20),NsnViews.dp(this,14),NsnViews.dp(this,20),NsnViews.dp(this,14));button.setBackgroundColor(Color.rgb(32,32,32));button.setOnFocusChangeListener((v,f)->{v.setBackgroundColor(f?Color.rgb(210,0,30):Color.rgb(32,32,32));v.animate().scaleX(f?1.08f:1f).scaleY(f?1.08f:1f).setDuration(100).start();});button.setOnClickListener(v->action.run());controlButtons.addView(button);}
