...
public final class PlayerActivity extends Activity {
    ...
    private void addTvControls(){
        ...
        controlButtons=new LinearLayout(this);controlButtons.setOrientation(LinearLayout.HORIZONTAL);controlButtons.setGravity(android.view.Gravity.CENTER);controls.addView(controlButtons,new LinearLayout.LayoutParams(-1,-2));
        ...
        LinearLayout secondaryButtons=new LinearLayout(this);secondaryButtons.setOrientation(LinearLayout.HORIZONTAL);secondaryButtons.setGravity(android.view.Gravity.CENTER);secondaryButtons.setPadding(0,NsnViews.dp(this,2),0,0);controls.addView(secondaryButtons,new LinearLayout.LayoutParams(-1,-2));
    ...
    private void showControls(boolean focusPlay){if(controls==null)return;controls.setVisibility(View.VISIBLE);controls.bringToFront();if(focusPlay&&!controlButtons.hasFocus())controlButtons.getChildAt(1).requestFocus();uiHandler.removeCallbacks(hideControls);uiHandler.postDelayed(hideControls,2800);}
    private void updateWebState(double position,double duration,boolean paused){if(resolved.get()||duration<=0)return;webPositionMs=(long)(position*1000);webDurationMs=(long)(duration*1000);resolver.setAlpha(1f);if(controls!=null){if(controls.getVisibility()!=View.VISIBLE)showControls(false);timeline.setProgress((int)Math.min(1000,webPositionMs*1000/Math.max(1,webDurationMs)));timeLabel.setText(formatTime(webPositionMs)+" / "+formatTime(webDurationMs));if(!controlButtons.hasFocus())controlButtons.getChildAt(1).requestFocus();}}
